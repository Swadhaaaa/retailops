...
const DepartmentAnalytics = () => {
  ...
  const priorityData = distribution('priority');
  const maxCategory = Math.max(1, ...categoryData.map(([, count]) => count));
  const maxPriority = Math.max(1, ...priorityData.map(([, count]) => count));

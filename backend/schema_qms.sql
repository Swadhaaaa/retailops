-- DuckDB-friendly QMS schema
-- This keeps the project on DuckDB while making the structure more organized and scalable.
...
CREATE INDEX IF NOT EXISTS idx_users_role_department_active ON users (role, department, is_active);
CREATE INDEX IF NOT EXISTS idx_tickets_status_priority_created ON tickets (status, priority, created_at);
CREATE INDEX IF NOT EXISTS idx_tickets_requester_created ON tickets (requester_id, created_at);
...
CREATE INDEX IF NOT EXISTS idx_ticket_status_history_ticket_changed ON ticket_status_history (ticket_id, changed_at);
